...
void masterStage_One(int numIndividuals, int numDimensions, double* individuals, double* values, char* evalScriptName){
	...
	for(int i = 0; i<numIndividuals; i++){
		printf("Hello from thread %d of %d at stage 1\n", omp_get_thread_num(), omp_get_num_threads());
	...
	printf("Values after the first stage:\n");	
	for(int i = 0; i<numIndividuals; i++){
		printf("[%d] => %lf\n", i, values[i]);
	}
}
...
void masterStage_Two(int numIndividuals, int numDimensions, double* individuals, double* values, int nProcs){
	...
	printf("Values after the second stage:\n");	
	for(int i = 0; i<numIndividuals; i++){
		printf("[%d] => %lf\n", i, values[i]);
	}
}
...
void master(int nProcs, char* evalScriptName){//printf("[MASTER] I'm here to execute %s with OpenMP\n", evalScriptName);
	...
	if(success){//----------------------
		printf("Individuals: %d ; Dimensions: %d\n", numIndividuals, numDimensions);
		for(int i = 0; i<numIndividuals; i++){
			for(int j = 0; j<numDimensions; j++){
				printf("%.1lf ", individuals[i*numDimensions + j]);
			}printf("\n");
		}
		//--------------------------
...
void worker(int rank, char* evalScriptName){//printf("[WORKER %d] I'm here to execute %s with my GPU\n", rank, evalScriptName);
	...
	MPI_Status status;
	while(1){
		printf("Worker %d Here ready for individuals of size %d\n", rank, numDimensions);
...
int main(int argc, char* argv[]){
	...
	if(size < 2){
		...
		if(rank == ORACLE_MASTER){
			if(argc < 2){
				printf("[MASTER] Usage: mpirun -np 2 name_evaluator_script\n");
			}else{
